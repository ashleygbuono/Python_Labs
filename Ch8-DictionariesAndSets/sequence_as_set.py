...
def find_common_unique_elements( iter1, iter2 ):
    ...
    for iter1_elem in iter1:
        if iter1_elem in iter2:
            set_with_common.add( iter1_elem )

    return set_with_common
...
def gen_return_random_unique_num_list( num_items ) :
    #have the function return a list of unique random numbers
    unique_random = {randint(0, 20) for elem in range(0, num_items)}
    # The expression        randint(0, 20)      generates a random #
    return list(unique_random)
...
def main( ):
    ...
    buncha_iters = [gen_return_random_unique_num_list(num_each_iterator) for row in range(0, num_iterators)]
