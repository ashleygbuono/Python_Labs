...
class BankAccount:
    ...
    def balance(self, new_bal):
        """
        Test if invalid balance for setter (-1000) prints diagnostic
         **** CODE STUFF HERE ****
         :param self: Instance of a BankAccount object
         :param new_bal: A dollar amount that potentially sets the previous self.balance parameter
         :return self._balance
         >>> acct_for_test = BankAccount(100, "J Doe", 1000)
         >>> acct_for_test.balance = 1000
    ...
    def make_deposit(self, dep_amt):
        ...
        Test if invalid balance for make_deposit prints diagnostic
         >>> acct_for_test = BankAccount(100, "J Doe", 1000)
        >>> acct_for_test.make_deposit(-1000)
        ...
        >>> existing = acct_for_test.balance
        >>> acct_for_test.make_deposit(200)
        >>> new = acct_for_test.balance
        >>> new == existing + 200
        True
    ...
    def transfer(self, trans_amt, other_acct):
        '''
        Test if same account holder issues diagnostic
        >>> acct_for_test = BankAccount(100, "J Doe", 1000)
        >>> diff_acct = BankAccount(100, "J X Doe", 1000)
        >>> diff_acct.transfer(300, acct_for_test)
